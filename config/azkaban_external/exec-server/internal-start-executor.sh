...
bash -c 'while true; do curl_response=`curl -k -w "%%{http_code}\\n" http://localhost:${azkaban_external_executor_port}/executor?action=activate -o /dev/null --connect-timeout 3 --max-time 5`; if [ $curl_response -ne "200" ]; then sleep 5; else break; fi done' &

export SID=$(curl -k -X POST --data "action=login&username=${admin_username}&password=${admin_password}" https://${azkaban_external_webserver_hostname}:${azkaban_external_webserver_port}  | jq -r '."session.id"')
bash -c 'sleep 20s; curl -k --data "session.id=$SID" https://${azkaban_external_webserver_hostname}:${azkaban_external_webserver_port}/executor?ajax=reloadExecutors' &
